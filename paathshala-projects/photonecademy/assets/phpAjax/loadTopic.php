<?php
	...
	if(!empty($courseId)&&!empty($subcourseId)&&!empty($subjectId)&&!empty($chapterId)){
		$loadTopic = mysqli_query($con, "SELECT `topic_id`, `courseid`, `subcourseid`, `subjectid`, `chapterid`, `topic_title` FROM `master_topics` WHERE `courseid` = '$courseId' AND `subcourseid` = '$subcourseId' AND `subjectid` = '$subjectId' AND `chapterid` = '$chapterId'");
?>
